...
export const Option: React.FC<OptionProps> = ({ countryCode, country, city, active }) => {
    return (
        <div className="h-14 w-full py-2 divide-x flex items-center border-b">
            <div className="h-full min-w-[3.25rem] flex justify-center items-center font-semibold">{countryCode}</div>
        ...
        <Listbox value={selected} onChange={setSelected}  >
            <div className="w-full relative">
                <Listbox.Button className="rounded-xl border h-14 w-full py-2 flex items-center hover:border-gray-600">
                    <div className="h-full min-w-[3.25rem] flex justify-center items-center font-semibold">{selected.countryCode}</div>
                    <div className="border-l pl-2 text-start">
                        <h5 className="text-sm font-medium">{selected.country}</h5>
                        <p className="text-xs">{selected.city}</p>
                    </div>
                </Listbox.Button>
                <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacountry-100" leaveTo="opacountry-0">
                    <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-[20]">
                        {dummyData.map((person: dummyDataType, personIdx) => (
                            <Listbox.Option key={personIdx} value={person}
                                ...
                                {({ selected }) => (
                                    <Option countryCode={person.countryCode} country={person.country} city={person.city} />
                                )}
                            </Listbox.Option>
                        ))}
                    </Listbox.Options>
...
const LocationSelect: React.FC<LocationSelectProps> = ({ type, activeLocation }) => {
  ...
  const handleSelect = (location: dummyDataType) => {
    ...
    function handleSearch(event: React.ChangeEvent<HTMLInputElement>) {
        ...
        setoptionsClone(dummyData.filter((location: dummyDataType) => location.country.toLowerCase().includes(searchValue)));
    }
              ...
              >
                ...
                <div className="border-l pl-3 text-start">
                  ...
                  <p className="text-xs text-gray-400 text-light">
                    {type === 'nationality' ? 'Nationality' : 'Visa ' + (type === 'visa-category' ? 'Category' : 'Country')}
                  </p>
                </div>
              </label>
            </div>
                  ...
                  {optionsClone.map((location: dummyDataType, locationIdx) => (
                    <li
                      ...
                      className={`
                        ...
                        ${location.country === selected.country && 'bg-green-100 text-green-900'}
                      `}
                      onClick={() => handleSelect(location)}
                    >
                      <Option countryCode={location.countryCode} country={location.country} city={location.city} />
                    </li>
                  ))}
                </ul>
              </div>
