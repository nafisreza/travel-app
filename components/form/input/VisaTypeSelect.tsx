...
export const Option: React.FC<OptionProps> = ({ visaType, active }) => {
  return (
...
const VisaSelect: React.FC<VisaSelectProps> = ({ activeVisa }) => {
  ...
  function handleSearch(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setOptionsClone(
      visaTypes.filter((visaType: string) =>
        visaType.toLowerCase().includes(searchValue)
      )
    );
  }
        ...
        <div className="block relative h-14 overflow-hidden rounded-xl border w-full hover:border-gray-400">
          ...
          >
            ...
            <div className="border-l pl-3 text-start">
              ...
              <p className="text-xs text-gray-400 text-light">Visa Type</p>
            </div>
          </label>
        </div>
        {focused && (
          <div className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-[20]">
            <ul>
              {optionsClone.map((visaType: string, index) => (
                <li
                  key={index}
                  className={`
